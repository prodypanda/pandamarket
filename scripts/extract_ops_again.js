...
const monolithFile = path.join('frontend/src/app/(admin)/settings/page.tsx');
const opsFile = path.join('frontend/src/app/(admin)/settings/operations/page.tsx');

if (fs.existsSync(monolithFile)) {
  let mono = fs.readFileSync(monolithFile, 'utf8');
  
  // Extract operations section
  let start = mono.indexOf('activeTab === \'operations\' &&');
  if (start > -1) {
    let section = mono.substring(start);
    let end = section.indexOf('</section>');
    // It has multiple sections! Let's just find the end of the operations tab block.
    // In `fix_all.js`, I extracted it using regex, but it had syntax errors.
    // Actually, I can just use the `operations` template with `SettingsFormHelpers`.
  }
}

// Since extracting with AST is hard, let's just do a clean rewrite of the operations page based on DEFAULT_SETTINGS keys related to operations.
const opsCode = `'use client';
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
      ...
      <section className="rounded-[2rem] border border-slate-200/70 bg-white p-8 shadow-xl shadow-slate-200/40 mb-8">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {renderToggle(formProps, { key: 'maintenance_enabled', label: 'Maintenance Mode', description: 'Enable to show maintenance page to visitors.' })}
          {renderTextInput(formProps, 'maintenance_message', 'Maintenance Message', 'We are currently performing maintenance. Please check back later.')}
          {renderTextInput(formProps, 'maintenance_bypass_key', 'Bypass Key', 'Secret key to access site during maintenance')}
        </div>
      </section>
      ...
      <section className="rounded-[2rem] border border-slate-200/70 bg-white p-8 shadow-xl shadow-slate-200/40 mb-8">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {[
            { key: 'notifications_in_app_enabled' as const, label: 'In-App Notifications', description: 'Show notification bell in admin dashboard.' },
            { key: 'notifications_realtime_enabled' as const, label: 'Real-time Updates', description: 'Use WebSocket for instant notifications.' },
            { key: 'notifications_email_enabled' as const, label: 'Email Notifications', description: 'Send critical alerts via email.' },
            { key: 'notifications_whatsapp_enabled' as const, label: 'WhatsApp Notifications', description: 'Send alerts via WhatsApp.' },
        ...
        <div className="mt-6 space-y-1.5">
          ...
          <select
            value={form.watch("notifications_whatsapp_provider") || 'twilio'}
            onChange={(e) => form.setValue("notifications_whatsapp_provider" as any, e.target.value, {shouldDirty: true})}
...
fs.writeFileSync(opsFile, opsCode);
console.log('Recreated Operations tab cleanly');
