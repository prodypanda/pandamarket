...
export class AnalyticsService {
  ...
  public async getPageViewsAnalytics(params: AnalyticsQueryParams): Promise<any> {
    ...
    try {
      ...
      topCountries = await Promise.all(topCountriesRes.rows.map(async (r: any) => {
        const vCount = Number(r.views_count || 0);
        ...
        let ipAddresses: any[] = [];
        try {
          const ipsRes = await query(`
            SELECT
              COALESCE(NULLIF(metadata->>'ip_address_masked', ''), CONCAT('visitor:', LEFT(COALESCE(visitor_hash, id), 10))) AS ip,
              COALESCE(NULLIF(metadata->>'city', ''), NULLIF(metadata->>'region', ''), $2) AS city,
              COALESCE(NULLIF(metadata->>'isp', ''), NULLIF(referrer_domain, ''), 'Direct / first-party') AS isp,
              COALESCE(device_type, 'web') AS device_type,
              COUNT(id)::int AS views_count,
              MAX(occurred_at) AS last_active,
              BOOL_OR(occurred_at >= NOW() - INTERVAL '15 minutes') AS is_active_now
            FROM pd_marketplace_analytics_event
            WHERE COALESCE(NULLIF(metadata->>'country_code', ''),
              CASE
                WHEN locale ILIKE '%-TN' OR locale ILIKE '%_TN' OR locale ILIKE 'tn%' THEN 'TN'
                WHEN locale ILIKE '%-FR' OR locale ILIKE '%_FR' OR locale ILIKE 'fr%' THEN 'FR'
                WHEN locale ILIKE '%-US' OR locale ILIKE '%_US' THEN 'US'
            ...
            ${dateFilterAnd}
            GROUP BY 1, 2, 3, 4
            ORDER BY views_count DESC, last_active DESC
            LIMIT 8
          `, [code, name]);
      ...
      topCountries = await Promise.all(topCountriesRes.rows.map(async (r: any) => {
        ...
        try {
          const ipsRes = await query(`
            ...
            FROM pd_marketplace_analytics_event
            WHERE occurred_at >= NOW() - INTERVAL '24 hours'
              AND COALESCE(NULLIF(metadata->>'country_code', ''),
                CASE
                  WHEN locale ILIKE '%-TN' OR locale ILIKE '%_TN' OR locale ILIKE 'tn%' THEN 'TN'
                  WHEN locale ILIKE '%-FR' OR locale ILIKE '%_FR' OR locale ILIKE 'fr%' THEN 'FR'
                  WHEN locale ILIKE '%-US' OR locale ILIKE '%_US' THEN 'US'
