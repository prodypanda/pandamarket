...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // ⚡ Bolt: Cache store details outside the import loop to prevent N+1 query performance bottleneck
    // This optimization ensures that processing up to 500 items only hits the store DB once
    const store = await storeService.getById(req.user!.store_id!);
