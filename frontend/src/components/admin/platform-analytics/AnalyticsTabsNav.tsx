...
export function AnalyticsTabsNav({ activeTab, tabLoading, onTabChange }: AnalyticsTabsNavProps) {
  ...
  return (
    <nav
      ...
      className="w-full lg:w-80 xl:w-80 flex-shrink-0 space-y-6"
    >
      {/* Mobile Horizontal Scroll Fallback (< lg screens) */}
      <div className="lg:hidden flex gap-2 overflow-x-auto pb-2 border-b border-slate-200 dark:border-slate-800 no-scrollbar touch-pan-x">
        {TAB_GROUPS.flatMap(g => g.tabs).map((tab) => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-2.5 rounded-xl text-xs font-black whitespace-nowrap flex items-center gap-2 border transition-all ${
                isSelected
                  ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white border-indigo-500 shadow-md shadow-indigo-500/20 scale-[1.02]'
            ...
            >
              ...
              {tab.badge && (
                <span className={`px-1.5 py-0.2 text-[9px] font-black rounded uppercase ${
                  isSelected ? 'bg-emerald-400 text-slate-950' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300'
                }`}>
                  {tab.badge}
                </span>
              )}
