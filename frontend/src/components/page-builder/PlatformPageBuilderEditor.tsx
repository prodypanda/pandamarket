...
export function PageBuilderEditor({
  ...
  const savedSectionsStorageKey = useMemo(() => `pd_page_builder_saved_sections_platform`, []);
  ...
  const loadVersions = useCallback(async () => {
    ...
    try {
      const res = await fetchWithCsrf(`/api/pd/marketplace/cms/${pageId}/versions`, { credentials: 'include' });
      if (!res.ok) {
  ...
  const restoreVersion = useCallback(async (version: PageBuilderVersion) => {
    ...
    setFeedback(null);
    try {
      const res = await fetchWithCsrf(`/api/pd/marketplace/cms/${pageId}/versions/${version.id}/restore`, {
        method: 'POST',
        credentials: 'include',
      });
  ...
  const loadStoreMediaAssets = useCallback(async (targetEditor = editorRef.current, showSuccess = false) => {
    ...
    try {
      const res = await fetchWithCsrf('/api/pd/platform-media?limit=100', { credentials: 'include' });
      if (!res.ok) {
  ...
  const persistPage = useCallback(async (opts: {
    ...
    try {
      ...
      const res = await fetchWithCsrf(`/api/pd/marketplace/cms/${pageId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  }, [globalStyles, initialData, loadVersions, onSave, pageId, pageSettings, recordActivity, refreshEditorSnapshot,  t]);
  ...
  const handlePreview = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchWithCsrf(`/api/pd/marketplace/cms/${pageId}/preview`, {
        method: 'POST',
        credentials: 'include',
      });
      const data = await res.json().catch(() => null) as {
  ...
  }, [editorReady, pageId, persistPage, previewing,  t]);
      ...
      <div className="h-16 bg-[#FBF8F1] border-b border-[#D6B779]/40 flex items-center justify-between px-5 flex-shrink-0 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={previewing || saving || publishing || !editorReady || false}
            ...
            <details>
              ...
              <div className="mt-3 overflow-hidden rounded-2xl border border-[#E4D8C6] bg-white shadow-sm">
                ...
                <div className="p-3">
                  ...
                  <p className="mt-2 truncate text-[10px] font-bold uppercase tracking-wider text-[#8A6F3D]">{"platform"}.{getMarketplaceDomain()}</p>
                </div>
              </div>
            </details>
          </div>
