...
interface PageBuilderEditorProps {
  pageId: string;
  /** 'store' (default): /api/pd/page-builder + storefront preview. 'platform': /api/pd/marketplace/cms + hub preview. */
  mode?: 'store' | 'platform';
  storeId?: string;
...
export function PageBuilderEditorCore({
  pageId,
  mode = 'store',
  storeId,
  storeHost,
  ...
  // Audit E17 dedup: variant switches every API surface + preview target.
  const isPlatform = mode === 'platform';
  const apiBase = isPlatform ? '/api/pd/marketplace/cms' : '/api/pd/page-builder/pages';
  const savedSectionsStorageKey = useMemo(
    () => `pd_page_builder_saved_sections_${isPlatform ? 'platform' : storeId || 'store'}`,
    [isPlatform, storeId],
  );
  ...
  const loadVersions = useCallback(async () => {
    ...
    try {
      const res = await fetchWithCsrf(`${apiBase}/${pageId}/versions`, { credentials: 'include' });
      if (!res.ok) {
  ...
  const restoreVersion = useCallback(async (version: PageBuilderVersion) => {
    ...
    setFeedback(null);
    try {
      const res = await fetchWithCsrf(`${apiBase}/${pageId}/versions/${version.id}/restore`, {
        method: 'POST',
        credentials: 'include',
      });
  ...
  const loadStoreMediaAssets = useCallback(async (targetEditor = editorRef.current, showSuccess = false) => {
    ...
    try {
      const res = await fetchWithCsrf(
        isPlatform ? '/api/pd/platform-media?limit=100' : '/api/pd/stores/me/media?limit=100',
        { credentials: 'include' },
      );
      if (!res.ok) {
  ...
  const persistPage = useCallback(async (opts: {
    ...
    try {
      ...
      const res = await fetchWithCsrf(`${apiBase}/${pageId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const handlePreview = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchWithCsrf(`${apiBase}/${pageId}/preview`, {
        method: 'POST',
        credentials: 'include',
      });
      const data = await res.json().catch(() => null) as {
      ...
      if (isPlatform) {
        previewPath = `/hub/pages/${encodeURIComponent(data.page.slug)}`;
      } else if (data.page.is_homepage) {
        previewPath = `/store/${encodeURIComponent(host ?? '')}`;
      } else {
        previewPath = `/store/${encodeURIComponent(host ?? '')}/pages/${encodeURIComponent(data.page.slug)}`;
      }
      ...
      <div className="h-16 bg-[#FBF8F1] border-b border-[#D6B779]/40 flex items-center justify-between px-5 flex-shrink-0 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={previewing || saving || publishing || !editorReady || (!isPlatform && !storeHost)}
            ...
            <details>
              ...
              <div className="mt-3 overflow-hidden rounded-2xl border border-[#E4D8C6] bg-white shadow-sm">
                ...
                <div className="p-3">
                  ...
                  <p className="mt-2 truncate text-[10px] font-bold uppercase tracking-wider text-[#8A6F3D]">{isPlatform ? 'platform' : storeHost || t('dashboardPages.pageBuilder.editor.storeDefault')}.{getMarketplaceDomain()}</p>
                </div>
              </div>
            </details>
          </div>
