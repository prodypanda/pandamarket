  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-3 sm:p-4">
        <div className="max-h-[92vh] w-full max-w-6xl overflow-hidden rounded-[2rem] bg-white shadow-2xl flex flex-col">
          {/* Header */}
          <div className="border-b border-gray-100 bg-gray-50/70 px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <div className="flex flex-wrap items-center gap-2.5">
                <h2 className="text-lg font-black text-gray-900">{t('dashboardPages.orders.orderDetails')}</h2>
                <span className="inline-flex items-center rounded-lg bg-gray-200/80 px-2.5 py-0.5 font-mono text-xs font-bold text-gray-700">
                  #{order.id.slice(-8).toUpperCase()}
                </span>
                {order.fulfillment_status && (
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-extrabold border ${fulfillmentColor(order.fulfillment_status)}`}>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <a
                ...
                className="inline-flex items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-3 py-1.5 text-xs font-extrabold text-gray-700 hover:border-emerald-500 hover:text-emerald-700 transition-colors shadow-xs"
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl border border-amber-200 bg-amber-50 px-3 py-1.5 text-xs font-extrabold text-amber-800 transition-colors hover:bg-amber-100"
              >
          ...
          <div className="flex border-b border-gray-100 bg-white px-6 overflow-x-auto gap-2 py-2">
            {[
              { id: 'overview', label: 'Vue d\'ensemble', icon: <Layers className="w-4 h-4" /> },
              { id: 'items', label: `Articles (${order.items?.length || 0})`, icon: <ShoppingBag className="w-4 h-4" /> },
              { id: 'shipping', label: 'Expédition & Transport', icon: <Truck className="w-4 h-4" /> },
              ...(order.payment_gateway === 'cod' ? [{ id: 'cod', label: 'Radar COD & Diagnostic', icon: <ShieldAlert className="w-4 h-4" /> }] : []),
              { id: 'notes', label: 'Facture & Notes', icon: <StickyNote className="w-4 h-4" /> },
            ].map((tab) => {
