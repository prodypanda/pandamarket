...
export const ProductDetailV1: React.FC<ProductDetailProps> = ({
  ...
  const effectiveLocale: Locale = isValidLocale(locale) ? locale : DEFAULT_LOCALE;
  const tx = (key: string, values?: Record<string, string | number>) => translate(effectiveLocale, key, values);
  ...
  return (
    <div data-testid="product-detail-v1">
      ...
      {isAliExpress && (
        <div className="mb-6 overflow-hidden rounded-[1.75rem] bg-gradient-to-r from-[#ff4747] via-[#ff5f2e] to-[#ff8a00] p-[1px] shadow-xl shadow-orange-900/10">
          <div className="flex flex-col gap-3 rounded-[1.7rem] bg-white/95 px-5 py-4 text-sm sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link
              href="/hub/search"
              className="inline-flex items-center justify-center rounded-full bg-[#fff1e8] px-4 py-2 text-xs font-black text-[#ff4747] hover:bg-[#ffe1d1]"
            >
              Continue shopping
            </Link>
          </div>
      ...
      {/* Breadcrumb */}
      <nav
        className={`mb-8 flex items-center gap-2 rounded-full px-4 py-3 text-sm ${
          isAliExpress
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.08fr_0.92fr] gap-8 mb-16">
        ...
        <div className={`lg:sticky lg:top-24 h-fit p-6 sm:p-8 ${cardClass}`}>
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            <span
              className={`rounded-full px-3 py-1 text-xs font-bold ${
                isAliExpress ? 'bg-[#fff1e8] text-[#7a2d11]' : 'bg-gray-100 text-gray-600'
              }`}
            >
              {product.status}
            </span>
          ...
          {/* Rating */}
          <div
            className={`mb-5 inline-flex flex-wrap items-center gap-2 rounded-full px-3 py-2 ${
          ...
          {wholesalePricing && (
            <div
              className={`mb-6 rounded-[1.75rem] p-5 ${
                isAliExpress ? 'border border-orange-100 bg-orange-50/70' : 'border border-emerald-100 bg-emerald-50/70'
              }`}
            >
              <p className="text-xs font-black uppercase tracking-[0.2em] text-gray-400">
                {tx('productWholesale.publicTitle')}
              </p>
              <p className="mt-1 text-sm font-semibold text-gray-700">{tx('productWholesale.publicSubtitle')}</p>
            ...
            {[
              ...
              <div
                ...
                className={`rounded-2xl p-3 ${
                  isAliExpress ? 'bg-orange-50/70 text-[#7a2d11]' : 'bg-gray-50 text-gray-700'
                }`}
              >
          ...
          {isPhysicalProduct && product.inventory_quantity !== undefined && (
            <p
              className={`mb-6 inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm font-bold ${
                product.inventory_quantity > 0 ? accentBgSoft : 'bg-red-50 text-red-600'
              }`}
            >
              ...
              {product.tags.map((tag) => (
                <span
                  ...
                  className={`px-3 py-1 text-xs font-bold rounded-full ${
                    isAliExpress ? 'bg-orange-50 text-[#7a2d11]' : 'bg-gray-100 text-gray-600'
                  }`}
                >
                  {tag}
                </span>
      ...
      <div className={`mb-16 p-6 sm:p-8 ${cardClass}`}>
        <h2 className="text-xl font-black text-gray-900 mb-6">Avis clients ({reviewCount})</h2>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
            {similarProducts.map((p) => (
              <Link key={p.id} href={getHubProductHref(p)} className={`group overflow-hidden ${classes.card}`}>
                <div className={`aspect-square relative overflow-hidden ${isAliExpress ? 'bg-orange-50' : 'bg-gray-100'}`}>
                  {p.category && (
                    <span
                      className={`absolute left-3 top-3 z-10 rounded-full px-3 py-1 text-[11px] font-bold shadow-sm ${
                        isAliExpress ? 'bg-white/95 text-[#ff4747]' : 'bg-white/90 text-gray-700'
                      }`}
                    >
                      {p.category}
                    </span>
                ...
                <div className="p-4">
                  <h3
                    className={`font-bold text-gray-900 text-sm mb-1 line-clamp-2 transition-colors ${classes.primaryTextHover}`}
                  >
                    {p.title}
                  </h3>
