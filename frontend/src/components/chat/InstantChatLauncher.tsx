...
function normalizeBubbleSettings(
...
export function InstantChatLauncher({
  storeContext,
  marketplaceTheme = 'panda',
}: InstantChatLauncherProps) {
  ...
  const isOwnStore = Boolean(
    storeContext?.storeId && currentUser?.store_id === storeContext.storeId,
  );
  ...
  const actions = useMemo(() => {
    ...
    if (isBuyer || isSeller) {
      items.push({
        ...
        helper: 'Chat with the superadmin support team.',
        icon: 'shield',
      });
    }
    if (storeContext && !isOwnStore) {
      if (isBuyer) {
        items.push({
          target: 'store',
          label: `${storeContext.storeName} manager`,
          helper: 'Ask this seller about products, stock, or orders.',
          icon: 'store',
        });
      } else if (isSeller) {
        items.push({
          target: 'store',
          label: `${storeContext.storeName} manager`,
          helper: 'Start a seller-to-seller conversation.',
          icon: 'store',
        });
      } else if (isAdmin) {
        items.push({
          target: 'store',
          label: `${storeContext.storeName} owner`,
          helper: 'Contact the store owner directly.',
          icon: 'store',
        });
      }
    }
  ...
  const startChat = async (target: ChatTarget, forceNew = false) => {
    ...
    try {
      const subject =
        storeContext?.productTitle ||
        (storeContext ? `Conversation with ${storeContext.storeName}` : 'Marketplace support');
  ...
  const launcher = (
    <div
      className={`fixed bottom-5 z-50 ${bubbleSettings.position === 'bottom-left' ? 'left-5' : 'right-5'}`}
    >
      {open && (
        <div className="mb-3 w-[min(360px,calc(100vw-2.5rem))] overflow-hidden rounded-[1.75rem] border border-gray-100 bg-white shadow-2xl shadow-slate-900/20">
          <div
            className={`p-4 text-white ${isAliExpress ? 'bg-gradient-to-r from-[#ff4747] to-[#ff8a00]' : 'bg-gradient-to-r from-slate-950 to-[#16C784]'}`}
          >
            ...
            {existingConversationId && (
              <div className="rounded-2xl border border-amber-100 bg-amber-50 p-3">
                <p className="text-sm font-black text-amber-900">
                  You already have an open chat with this seller.
                </p>
                <p className="mt-1 text-xs font-semibold text-amber-700">
                  Resume the old conversation or create a new one.
                </p>
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => redirectToConversation(existingConversationId)}
                  ...
                  <button
                    type="button"
                    onClick={() => startChat('store', true)}
                    className="rounded-xl bg-white px-3 py-2 text-xs font-black text-amber-800 ring-1 ring-amber-200"
                  >
                    New chat
                  </button>
                </div>
