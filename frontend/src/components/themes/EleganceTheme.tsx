...
export function EleganceTheme({ theme, storeName, products = [], branding }: ThemeProps) {
  ...
  const logoUrl = getStoreBrandLogo(
    branding,
    getLogoSurfaceForColor(tc.colors.headerBg, getStoreThemeLogoSurface(theme.id)),
  );
    ...
    >
      ...
      <header className="border-b border-gray-200/60">
        <div className="max-w-6xl mx-auto px-6 py-10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-5">
            <Search
              className="w-5 h-5 text-gray-400 hover:text-gray-700 transition-colors cursor-pointer"
              strokeWidth={1.5}
            />
        ...
        <nav className="hidden md:flex justify-center gap-10 pb-6 text-xs tracking-[0.2em] uppercase text-gray-500">
          ...
          <Link
            href={`${branding?.store_path_base || ''}/pages/contact`}
            className="hover:text-gray-900 transition-colors"
          >
            Contact
          </Link>
      ...
      <footer className="border-t border-gray-200/60 py-16 text-center">
        <p className="text-xs text-gray-400 tracking-wide">
          ...
          <PoweredByMarketplace
            branding={branding}
            linkClassName="text-[#16C784] hover:underline"
          />
        </p>
      </footer>
    </div>
