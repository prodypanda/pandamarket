...
export function KidsTheme({ theme, storeName, products = [], branding }: ThemeProps) {
  const primary = branding?.primary_color || '#FF6B9D';
  const yellow = '#FFD93D';
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#FFFBF0', color: '#2D2D2D' }}>
      ...
      <header className="border-b-4" style={{ borderColor: yellow }}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex gap-6 text-sm font-bold" style={{ color: primary }}>
            ...
            <a href="#" className="hover:opacity-70 transition-opacity">New</a>
            <a href="#" className="hover:opacity-70 transition-opacity">Sale</a>
          </nav>
          <Link href="/hub/cart" className="flex items-center gap-1 px-4 py-2 rounded-full text-sm font-bold text-white" style={{ backgroundColor: primary }}>
      ...
      <main id="products" className="max-w-7xl mx-auto px-6 pb-24">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {dp.map((p) => (
            <Link key={p.id} href={`/hub/products/${p.id}`} className="group block rounded-3xl overflow-hidden bg-white shadow-sm hover:shadow-lg transition-all border-2" style={{ borderColor: `${primary}15` }}>
              <div className="aspect-square overflow-hidden" style={{ backgroundColor: '#FFF5F8' }}>
                {p.images?.[0]?.url ? <img src={p.images[0].url} alt={p.title} className="w-full h-full object-cover group-hover:scale-110 group-hover:rotate-1 transition-transform duration-500" /> : (
              ...
              <div className="p-3 text-center">
                ...
                <p className="text-sm font-black mt-1" style={{ color: primary }}>{p.price.toFixed(3)} TND</p>
              </div>
            </Link>
          ))}
        </div>
