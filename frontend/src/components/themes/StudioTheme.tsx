...
export function StudioTheme({ theme, storeName, products = [], branding }: ThemeProps) {
  const accent = branding?.primary_color || '#8B5CF6';
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#FAFAFA', color: '#1A1A1A' }}>
      ...
      <header className="border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-8 flex items-center justify-between">
          ...
          <nav className="hidden md:flex gap-8 text-sm text-gray-500">
            <a href="#" className="hover:text-gray-900 transition-colors">Gallery</a>
            <a href="#" className="hover:text-gray-900 transition-colors">About</a>
            <a href="#" className="hover:text-gray-900 transition-colors">Contact</a>
          </nav>
          <Link href="/hub/cart"><ShoppingBag className="w-5 h-5 text-gray-400 hover:text-gray-700 transition-colors" /></Link>
        </div>
      </header>
      ...
      <main id="products" className="max-w-7xl mx-auto px-6 pb-24">
        ...
        <div className="columns-2 md:columns-3 gap-4 space-y-4">
          {dp.map((p, i) => (
            <Link key={p.id} href={`/hub/products/${p.id}`} className="group block break-inside-avoid rounded-lg overflow-hidden bg-white shadow-sm hover:shadow-lg transition-all">
              <div className={`overflow-hidden ${i % 3 === 0 ? 'aspect-[3/4]' : i % 3 === 1 ? 'aspect-square' : 'aspect-[4/3]'}`} style={{ backgroundColor: '#F0F0F0' }}>
                {p.images?.[0]?.url ? <img src={p.images[0].url} alt={p.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" /> : (
              ...
              <div className="p-4">
                ...
                <p className="text-sm mt-1" style={{ color: accent }}>{p.price.toFixed(3)} TND</p>
              </div>
            </Link>
          ))}
        </div>
