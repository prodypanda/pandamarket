...
import {
  getMarketplaceThemeClasses,
  type MarketplaceThemeSettings,
...
import {
  getMarketplaceStoreProductHref,
  type MarketplaceStoreData,
  type MarketplaceStoreProduct,
} from './MarketplaceStorefront';
...
export function MarketplaceStoreProductDetail({
  ...
  const wholesalePricing =
    sellerType === 'wholesaler' || sellerType === 'hybrid'
  ...
  return (
    <div className={`min-h-screen ${classes.pageSoft}`}>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <nav
          className={`mb-8 flex items-center gap-2 rounded-full px-4 py-3 text-sm ${isAliExpress ? 'border border-orange-100 bg-white/80 text-gray-500 shadow-sm shadow-orange-900/5' : 'text-gray-500'}`}
        >
        ...
        <div className="mb-16 grid grid-cols-1 gap-8 lg:grid-cols-[1.08fr_0.92fr]">
          <div
            className={
              isAliExpress ? 'rounded-[2rem] bg-white p-3 shadow-xl shadow-orange-900/5' : ''
            }
          >
            <ProductGallery
              title={product.title}
          ...
          <div className={`h-fit p-6 sm:p-8 lg:sticky lg:top-24 ${cardClass}`}>
            <div className="mb-4 flex flex-wrap gap-2">
              {product.category && (
                <Link
                  href={categoryHref}
                  className={`rounded-full px-3 py-1 text-xs font-black ${classes.primarySoft}`}
                >
                  {product.category}
                </Link>
              )}
              <span
                className={`rounded-full px-3 py-1 text-xs font-black ${isAliExpress ? 'bg-[#fff1e8] text-[#7a2d11]' : 'bg-gray-100 text-gray-600'}`}
              >
                {product.status}
              </span>
            </div>
            ...
            <div
              className={`mb-5 inline-flex flex-wrap items-center gap-2 rounded-full px-3 py-2 ${isAliExpress ? 'bg-orange-50 text-[#7a2d11]' : 'bg-gray-50 text-gray-600'}`}
            >
              <div className="flex items-center">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    key={star}
                    className={`h-5 w-5 ${star <= Math.round(numericRating) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
                  />
                ))}
              </div>
            ...
            <div
              className={`mb-6 rounded-[1.75rem] p-5 ${isAliExpress ? 'border border-orange-100 bg-gradient-to-br from-[#fff7f2] via-white to-white' : 'border border-gray-100 bg-gray-50'}`}
            >
              <p className="text-xs font-black uppercase tracking-[0.2em] text-gray-400">
                Marketplace price
              </p>
              <p className={`mt-1 text-4xl font-black sm:text-5xl ${classes.primaryText}`}>
            ...
            {wholesalePricing && (
              <div
                className={`mb-6 rounded-[1.75rem] p-5 ${isAliExpress ? 'border border-orange-100 bg-orange-50/70' : 'border border-emerald-100 bg-emerald-50/70'}`}
              >
                <p className="text-xs font-black uppercase tracking-[0.2em] text-gray-400">
                  {tx('productWholesale.publicTitle')}
                </p>
                <p className="mt-1 text-sm font-semibold text-gray-700">
                  {tx('productWholesale.publicSubtitle')}
                </p>
                <p
                  className={`mt-2 inline-flex rounded-full bg-white px-3 py-1 text-xs font-black ${classes.primaryText}`}
                >
                ...
                <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {wholesalePricing.price_tiers?.map((tier) => (
                    <div
                      ...
                      className="rounded-2xl bg-white px-4 py-3 text-sm font-bold text-gray-800 shadow-sm"
                    >
                      ...
                      <span className={classes.primaryText}>
                        {tx('productWholesale.unitPriceLine', {
                          price: Number(tier.unit_price).toFixed(3),
                        })}
                      </span>
                    </div>
                  ))}
            ...
            <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div
                className={`rounded-2xl p-4 ${isAliExpress ? 'bg-orange-50/70 text-[#7a2d11]' : 'bg-gray-50 text-gray-700'}`}
              >
            ...
            {isPhysicalProduct && product.inventory_quantity !== undefined && (
              <p
                className={`mb-6 inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm font-bold ${product.inventory_quantity > 0 ? classes.primarySoft : 'bg-red-50 text-red-600'}`}
              >
            ...
            {product.tags && product.tags.length > 0 && (
              <div className="mb-6 flex flex-wrap gap-2">
                {product.tags.map((tag) => (
                  <span
                    key={tag}
                    className={`rounded-full px-3 py-1 text-xs font-bold ${isAliExpress ? 'bg-orange-50 text-[#7a2d11]' : 'bg-gray-100 text-gray-600'}`}
                  >
                    {tag}
                  </span>
                ))}
            ...
            <div
              className={`flex flex-col sm:flex-row items-stretch sm:items-center gap-3 rounded-[1.75rem] p-3 ${isAliExpress ? 'bg-[#fff7f2]' : 'bg-gray-50'}`}
            >
              ...
              <button
                ...
                className="rounded-xl border border-gray-300 bg-white p-3 transition-colors hover:bg-gray-50 flex items-center justify-center"
              >
          ...
          <section>
            <div className="mb-6 flex items-end justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Other marketplace products from this seller.
                </p>
              </div>
              <Link
                href={productsHref}
                className={`text-sm font-bold ${classes.primaryText} ${classes.primaryTextHover}`}
              >
            ...
            <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {relatedProducts.map((relatedProduct) => {
                const imageUrl =
                  relatedProduct.thumbnail || getImageUrl(relatedProduct.images?.[0]);
                return (
                  <Link
                    key={relatedProduct.id}
                    href={getMarketplaceStoreProductHref(storeHost, relatedProduct)}
                    className={`group overflow-hidden ${classes.card}`}
                  >
                    <div
                      className={`relative aspect-square overflow-hidden ${isAliExpress ? 'bg-orange-50' : 'bg-gray-100'}`}
                    >
                      {imageUrl ? (
                        <div
                          ...
                          style={{
                            backgroundImage: `url(${getResizedImageUrl(imageUrl, 'large')})`,
                          }}
                        />
                      ) : (
                    ...
                    <div className="p-4">
                      <h3
                        className={`mb-1 line-clamp-2 text-sm font-bold text-gray-900 transition-colors ${classes.primaryTextHover}`}
                      >
                        {relatedProduct.title}
                      </h3>
                      <p className={`font-black ${classes.primaryText}`}>
