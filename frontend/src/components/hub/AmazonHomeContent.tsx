...
export function AmazonHomeContent({ trendingProducts, categories, marketplaceSettings }: AmazonHomeContentProps) {
  ...
  const sponsoredBrands = topSellers.slice(0, 3);
  const lightningDeals = trendingProducts.slice(0, 8);
  ...
  return (
    <div className="bg-[#e3e6e6] text-gray-900">
      ...
      {/* Category strip with hover panels (mega sidebar as horizontal strip on this variant) */}
        ...
        <div className="rounded-lg bg-white p-5 shadow-md">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <h2 className="flex items-center gap-2 text-xl font-black">
              <Zap className="h-5 w-5 fill-current" style={{ color: '#cc0c39' }} /> Lightning deals
            </h2>
        ...
        <section className="mx-auto max-w-7xl px-4 pb-2 sm:px-6 lg:px-8">
          <div className="rounded-lg bg-white p-5 shadow-md">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="text-xl font-black">Top sellers</h2>
            </div>
      ...
      {/* Recently viewed */}
      <RecentlyViewedRail accentClass="text-[#b12704]" />
            ...
            <div key={column.title}>
              ...
              <ul className="mt-3 space-y-2">
                {column.links.map((link) => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-xs font-semibold text-white/70 hover:text-white hover:underline">{link.label.replace('&amp;', '&')}</Link>
                  </li>
                ))}
              </ul>
            </div>
