...
export default function StorefrontCategoriesPage() {
  ...
  const uploadImage = async (file: File) => {
    ...
    if (!presignRes.ok) throw new Error(await getErrorMessage(presignRes, 'Failed to prepare image upload'));
  ...
  const deleteCategory = async (category: Category) => {
    ...
    if (!window.confirm(`Delete ${category.name}? Products will move to Non categorized products.`)) return;
    setSavingId(category.id);
    setError('');
    try {
      const res = await fetchWithCsrf(`/api/pd/stores/me/categories/${category.id}`, {
      ...
      <div className="relative overflow-hidden rounded-3xl border border-emerald-100 bg-gradient-to-br from-[#16C784] via-[#13b777] to-slate-950 p-6 sm:p-8 text-white shadow-xl shadow-emerald-900/10">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
          <div>
            ...
            <h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl">Storefront Categories</h1>
            <p className="mt-2 max-w-2xl text-sm text-white/80 sm:text-base">
              Organize your shop like WordPress with parent categories, descriptions, pictures, and protected default routing.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(320px,420px)_1fr] gap-6">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 h-fit">
          <div className="mb-5">
            ...
            <p className="mt-1 text-sm text-gray-500">Create a top-level category or attach it to a parent category.</p>
          </div>
          <div className="space-y-4">
            <div className="aspect-[4/3] rounded-2xl border border-dashed border-gray-300 bg-gray-50 overflow-hidden flex items-center justify-center">
              {form.image_url ? <img src={form.image_url} alt="Category" className="w-full h-full object-cover" /> : <ImageIcon className="w-10 h-10 text-gray-300" />}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-gray-100 bg-gray-50/70 px-5 py-4">
            <div>
              ...
              <p className="text-sm text-gray-500">{activeCount} active categories ready for your storefront navigation.</p>
            </div>
            ...
            <div className="divide-y divide-gray-100">
              {categories.map((category) => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <input value={category.short_description || ''} onChange={(event) => setCategories((current) => current.map((item) => item.id === category.id ? { ...item, short_description: event.target.value } : item))} placeholder="Short description" className="md:col-span-2 px-3 py-2 border border-gray-300 rounded-xl" />
                  <textarea value={category.long_description || ''} onChange={(event) => setCategories((current) => current.map((item) => item.id === category.id ? { ...item, long_description: event.target.value } : item))} placeholder="Long description" rows={3} className="md:col-span-2 px-3 py-2 border border-gray-300 rounded-xl resize-none" />
