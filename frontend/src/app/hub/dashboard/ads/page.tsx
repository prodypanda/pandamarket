...
export default function SellerAdsPage() {
  ...
  const setSellerPreset = (preset: 'today' | '7d' | '30d' | '90d') => {
    ...
    if (preset === 'today') {
      ...
      setFrom(new Date(Date.now() - 30 * 86400000).toISOString().slice(0, 10));
      setTo(today);
      setGranularity('daily');
    } else if (preset === '90d') {
      setFrom(new Date(Date.now() - 90 * 86400000).toISOString().slice(0, 10));
      setTo(today);
      setGranularity('daily');
    }
  };
  ...
  const redeemCoupon = async () => {
    ...
    if (!res.ok) {
      setError(data.error?.message || 'Invalid promo code');
      return;
    }
  ...
  const startRefill = async (event: React.FormEvent) => {
    ...
    if (refillGateway === 'manual_mandat') {
      if (!refillProofUrl.trim()) {
        setError(t('ads.proofUrlHint') || 'Please upload a picture of your payment receipt.');
        return;
      }
      ...
      if (!r.ok) {
        setError(d.error?.message || 'Unable to submit manual refill');
        return;
      }
      setRefilling(false);
      setRefillProofUrl('');
      setProofPreviewUrl('');
      setSuccessMsg(t('ads.mandatSubmitted') || 'Manual mandat refill submitted for admin review!');
      await load();
      return;
    }
    ...
    const d = await r.json();
    if (!r.ok) {
      setError(d.error?.message || 'Unable to start refill');
      return;
    }
  ...
  const action = async (id: string, name: string) => {
    const res = await fetchWithCsrf(`/api/pd/ads/campaigns/${id}/${name}`, {
      method: 'POST',
      credentials: 'include',
    });
    const data = await res.json();
    if (!res.ok) {
      setError(data.error?.message || 'Campaign action failed');
      return;
    }
    await load();
  };
  ...
  const hideCampaign = async (id: string) => {
    ...
    const res = await fetchWithCsrf(`/api/pd/ads/campaigns/${id}/hide`, {
      method: 'POST',
      credentials: 'include',
    });
    if (!res.ok) {
  ...
  const submitEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(
        'Campaign updated successfully! If it was approved, it has been resubmitted for admin review.',
      );
      await load();
    } catch (err) {
  ...
  return (
    <div dir={dir} className="space-y-7 p-4 sm:p-8">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black text-slate-950">
            {t('ads.center') || 'Ads & Sponsorship Center'}
          </h1>
          <p className="mt-2 text-sm text-slate-500">
            {t('ads.sponsorDesc') || 'Promote your brand and products across marketplace slots.'}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="rounded-xl border border-amber-500 bg-amber-50 px-5 py-3 text-sm font-black text-amber-800 cursor-pointer"
          >
          ...
          <button
            ...
            className="rounded-xl border border-emerald-600 bg-white px-5 py-3 text-sm font-black text-emerald-700 cursor-pointer"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-emerald-600/20 cursor-pointer"
          >
      ...
      {successMsg && (
        <div
          role="status"
          className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-bold text-emerald-700"
        >
          {successMsg}
        </div>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[
          [
            t('ads.availableBalance') || 'Available Balance',
            money(account?.balance, account?.currency),
            WalletCards,
          ],
          [
            t('ads.reserved') || 'Reserved',
            money(account?.reserved_balance, account?.currency),
            WalletCards,
          ],
          [
            t('ads.totalSpend') || 'Total Spend',
            money(account?.total_spend, account?.currency),
            BarChart3,
          ],
          [
            t('ads.activeCampaigns') || 'Active Campaigns',
            String(account?.active_campaigns || 0),
            Megaphone,
          ],
      ...
      {/* Low Balance Warning */}
      <div
        className={`rounded-2xl border p-5 ${Number(account?.balance || 0) < 5 ? 'border-red-200 bg-red-50' : 'border-amber-200 bg-amber-50'}`}
      >
        <p
          className={`font-black ${Number(account?.balance || 0) < 5 ? 'text-red-900' : 'text-amber-900'}`}
        >
          {Number(account?.balance || 0) < 5
            ? t('ads.lowBalanceTitle') || 'Low Ads Balance'
            : t('ads.prepaidAccountTitle') || 'Prepaid Account'}
        </p>
        <p
          className={`mt-1 text-sm ${Number(account?.balance || 0) < 5 ? 'text-red-700' : 'text-amber-800'}`}
        >
          {Number(account?.balance || 0) < 5
            ? t('ads.lowBalanceDesc') || 'Refill to prevent campaign pauses.'
            : t('ads.prepaidAccountDesc') || 'Your Ads account is funded.'}
        </p>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="font-black text-slate-900">
              {t('ads.performanceTitle') || 'Performance & Analytics'}
            </h2>
            <p className="text-xs text-slate-500">
              {t('ads.performanceDesc') || 'Track impressions, clicks, spend, and conversions.'}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select
              ...
              className="max-w-48 rounded-xl border px-3 py-2 text-xs font-semibold"
            >
            ...
            <select
              ...
              className="rounded-xl border px-3 py-2 text-xs font-bold text-slate-900 cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={() => load()}
              className="rounded-xl bg-slate-900 px-4 py-2 text-xs font-black text-white hover:bg-slate-800 cursor-pointer"
            >
              {t('ads.apply') || 'Apply'}
            </button>
          </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4 lg:grid-cols-8">
          {[
            ...
            [
              t('ads.convRate') || 'Conv Rate',
              `${((analytics?.conversion_rate || 0) * 100).toFixed(2)}%`,
            ],
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 p-5">
          <h2 className="font-black text-slate-900">
            {t('ads.campaignsTitle') || 'Your Ads Campaigns'}
          </h2>
        </div>
        {campaigns.length === 0 ? (
          <div className="p-12 text-center text-sm font-semibold text-slate-500">
            {t('ads.noCampaigns') || 'No campaigns created yet.'}
          </div>
        ) : (
          <div className="divide-y divide-slate-100">
            {campaigns.map((c) => {
              ...
              return (
                <div
                  key={c.id}
                  className="flex flex-wrap items-center justify-between gap-4 p-5 hover:bg-slate-50/70 transition"
                >
                  <div className="flex items-center gap-4">
                    {creative?.image_url ? (
                      <img
                        src={
                          creative.image_url ? getResizedImageUrl(creative.image_url, 'medium') : ''
                        }
                        alt=""
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`rounded-full px-2.5 py-0.5 text-[10px] font-black ${creative?.image_url ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}
                        >
                          Health: {creative?.image_url ? '95% Excellent' : '75% Needs Artwork'}
                        </span>
                      </div>
                      <p className="text-xs font-semibold text-slate-500">
                        {c.campaign_type.replaceAll('_', ' ')} · {money(c.spent_amount)}{' '}
                        {t('ads.spentOf') || 'spent of'} {money(c.total_budget)} · Rate:{' '}
                        {money(c.bid_amount)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="rounded-lg border bg-white p-2 text-slate-600 hover:bg-slate-50 transition cursor-pointer"
                      title="Preview Creative"
                      aria-label="Aperçu de la création"
                    >
                    ...
                    <button
                      ...
                      title="Hide/Archive Campaign"
                      aria-label="Masquer ou archiver la campagne"
                    >
                    ...
                    {c.status === 'draft' && (
                      <button
                        ...
                        className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-black text-white hover:bg-emerald-700 transition cursor-pointer"
                      >
                        {t('ads.submit') || 'Submit'}
                      </button>
                    )}
                    ...
                    {c.status === 'active' && (
                      <button
                        ...
                        className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-50 transition cursor-pointer"
                      >
                        {t('ads.pause') || 'Pause'}
                      </button>
                    )}
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b p-5">
          <h2 className="font-black text-slate-900">
            {t('ads.refillHistoryTitle') || 'Refill History'}
          </h2>
          <p className="text-xs text-slate-500">
            {t('ads.refillHistoryDesc') || 'Prepaid account top-up requests and receipts.'}
          </p>
        </div>
        {refills.length === 0 ? (
          <p className="p-8 text-center text-sm font-semibold text-slate-400">
            {t('ads.noRefills') || 'No refills executed.'}
          </p>
        ) : (
                ...
                {refills.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="p-4 font-bold capitalize text-slate-900">
                      {r.gateway === 'manual_mandat'
                        ? t('ads.mandatGateway') || 'Bank Transfer / Mandat'
                        : r.gateway}
                    </td>
                    ...
                    <td className="p-4">
                      {r.status === 'captured' ? (
                        <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-800">
                          {t('ads.captured') || 'Captured'}
                        </span>
                      ) : r.status === 'pending_review' ? (
                        <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-800">
                          {t('ads.pendingReview') || 'Pending Review'}
                        </span>
                      ) : r.status === 'rejected' ? (
                    ...
                    <td className="p-4">
                      {r.status === 'captured' ? (
                        <a
                          href={`/api/pd/ads/refills/${encodeURIComponent(r.id)}/receipt`}
                          className="rounded-lg bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700 hover:bg-emerald-100 transition"
                        >
                          {t('ads.download') || 'Receipt'}
                        </a>
                      ) : (
                        <span className="text-xs text-slate-400">
                          {t('ads.unavailable') || 'Unavailable'}
                        </span>
                      )}
                    </td>
                  </tr>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b p-5">
          <h2 className="font-black text-slate-900">
            {t('ads.transactionsTitle') || 'Ads Account Transactions'}
          </h2>
          <p className="text-xs text-slate-500">
            Ledger of all refills, campaign debits, refunds, and promo credits.
          </p>
        </div>
        {transactions.length === 0 ? (
          <p className="p-8 text-center text-sm font-semibold text-slate-400">
            {t('ads.noTransactions') || 'No transactions recorded.'}
          </p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
                ...
                {transactions.map((t) => (
                  <tr key={t.id}>
                    ...
                    <td className="p-4 text-xs font-bold text-slate-900">
                      {t.campaign_name || t.description || 'General Account Item'}
                    </td>
                    <td
                      className={`p-4 font-black ${Number(t.amount) >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
                    >
                      {Number(t.amount) >= 0 ? '+' : ''}
                      {money(t.amount)}
                    </td>
      ...
      {editingCampaign && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm">
          <form
            onSubmit={submitEdit}
            className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-3xl bg-white p-7 shadow-2xl space-y-4"
          >
            <div className="flex items-start justify-between border-b pb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Editing an approved ad will resubmit it for admin moderation.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="grid gap-3 sm:grid-cols-2 text-xs font-bold text-slate-700">
              <label className="sm:col-span-2 block">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-xl border p-3 text-sm font-semibold text-slate-900"
                />
              </label>
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(e) => setEditForm({ ...editForm, daily_budget: e.target.value })}
                  className="mt-1 w-full rounded-xl border p-3 text-sm font-semibold text-slate-900"
                />
              </label>
            ...
            <div className="flex gap-3 pt-3">
              <button
                type="button"
                onClick={() => setEditingCampaign(null)}
                className="flex-1 rounded-xl border py-3 text-xs font-black text-slate-700 hover:bg-slate-50 cursor-pointer"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="flex-1 rounded-xl bg-emerald-600 py-3 text-xs font-black text-white hover:bg-emerald-700 cursor-pointer"
              >
                Save & Update
              </button>
            </div>
            ...
            {previewCampaign.creatives?.[0] ? (
              <div className="overflow-hidden rounded-2xl border border-amber-200 bg-white shadow-sm p-4 space-y-3">
                {previewCampaign.creatives[0].image_url && (
                  <img
                    src={previewCampaign.creatives[0].image_url}
                    alt=""
                ...
                <div>
                  ...
                  <h3 className="mt-2 text-base font-black text-slate-900">
                    {previewCampaign.creatives[0].title}
                  </h3>
                  {previewCampaign.creatives[0].description && (
      ...
      {refilling && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm">
          <form
            onSubmit={startRefill}
            className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-3xl bg-white p-7 shadow-2xl space-y-5"
          >
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-2xl font-black text-slate-900">
                  {t('ads.refillModalTitle') || 'Refill Ads Balance'}
                </h2>
                <p className="text-xs text-slate-500">
                  {t('ads.refillModalDesc') || 'Prepay funds into your Ads account.'}
                </p>
              </div>
              <button
                type="button"
                onClick={() => setRefilling(false)}
            ...
            <div>
              <label className="block text-xs font-black uppercase tracking-wider text-slate-500">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-xl border border-slate-200 p-3 text-lg font-black text-slate-900 focus:border-emerald-500 focus:ring-emerald-500"
                />
              </label>
            ...
            <label className="block text-xs font-black uppercase tracking-wider text-slate-500">
              ...
              <select
                value={refillGateway}
                onChange={(e) => setRefillGateway(e.target.value)}
                className="mt-1 w-full rounded-xl border border-slate-200 p-3 text-sm font-bold text-slate-900 normal-case focus:border-emerald-500 focus:ring-emerald-500 cursor-pointer"
              >
                <option value="flouci">Flouci</option>
                <option value="konnect">Konnect</option>
                <option value="manual_mandat">
                  {t('ads.mandatGateway') || 'Bank Transfer / Mandat'}
                </option>
              </select>
            </label>
              ...
              <div className="space-y-4 rounded-2xl border border-amber-200 bg-amber-50/60 p-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-black text-amber-900 text-sm">
                    {t('ads.billingDetailsTitle') || 'Marketplace Billing Information'}
                  </h3>
                </div>
                ...
                <div className="space-y-1.5 text-xs text-slate-800 font-semibold bg-white p-3 rounded-xl border border-amber-100">
                  <p>
                    <span className="font-bold text-slate-500">
                      {t('ads.recipientName') || 'Beneficiary'}:
                    </span>{' '}
                    {billingInfo?.recipient_name || 'PandaMarket SARL'}
                  </p>
                  <p>
                    <span className="font-bold text-slate-500">{t('ads.bankName') || 'Bank'}:</span>{' '}
                    {billingInfo?.bank_name || 'STB (Société Tunisienne de Banque)'}
                  </p>
                  <p className="font-mono text-slate-900">
                    <span className="font-bold text-slate-500 font-sans">
                      {t('ads.rib') || 'RIB'}:
                    </span>{' '}
                    {billingInfo?.rib || '10 000 0000000000000 00'}
                  </p>
                  <p className="font-mono text-slate-900">
                    <span className="font-bold text-slate-500 font-sans">
                      {t('ads.iban') || 'IBAN'}:
                    </span>{' '}
                    {billingInfo?.iban || 'TN59 1000 0000 0000 0000 0000'}
                  </p>
                  <p>
                    <span className="font-bold text-slate-500">{t('ads.cinNumber') || 'CIN'}:</span>{' '}
                    {billingInfo?.cin || '01234567'} ({billingInfo?.city || 'Tunis'})
                  </p>
                  {billingInfo?.phone && (
                    <p>
                      <span className="font-bold text-slate-500">
                        {t('ads.phone') || 'Support Phone'}:
                      </span>{' '}
                      {billingInfo.phone}
                    </p>
                  )}
                ...
                <div className="space-y-2">
                  <label className="block text-xs font-black uppercase text-slate-700">
                    {t('ads.proofImage') || 'Proof Picture Upload'}
                  </label>
                  {uploadingFile ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5 text-xs font-black text-emerald-800">
                            <CheckCircle2 className="h-4 w-4 text-emerald-600" /> Uploaded
                            successfully
                          </div>
                          <button
                            type="button"
                            ...
                            className="mt-1 text-[11px] font-bold text-red-600 underline cursor-pointer"
                          >
                            Change picture
                          </button>
                        </div>
                    ...
                    <label className="flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-amber-300 bg-white p-6 text-center cursor-pointer hover:border-emerald-500 transition">
                      ...
                      <span className="text-xs font-bold text-slate-700">
                        {t('ads.clickToUpload') || 'Click to select picture'}
                      </span>
                      <input
                        type="file"
                        accept="image/*"
            ...
            <button
              ...
              className="w-full rounded-xl bg-emerald-600 py-3.5 text-sm font-black text-white hover:bg-emerald-700 disabled:opacity-50 transition cursor-pointer shadow-lg shadow-emerald-600/20"
            >
              {refillGateway === 'manual_mandat'
                ? t('ads.submitMandat') || 'Submit Mandat Proof'
                : t('ads.proceedToPayment') || 'Proceed to Payment'}
            </button>
          </form>
        </div>
      )}
