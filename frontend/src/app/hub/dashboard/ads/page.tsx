...
export default function AdsDashboardPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [a,c,p,t,n] = await Promise.all([
        ...
        fetchWithCsrf(`/api/pd/ads/analytics?from=${from}&to=${to}`, { credentials:'include' }),
      ]);
      if (!a.ok || !c.ok || !p.ok || !t.ok || !n.ok) throw new Error('Unable to load Ads data');
      const [ad,cd,pd,td,nd] = await Promise.all([a.json(),c.json(),p.json(),t.json(),n.json()]);
      setAccount(ad.account); setCampaigns(cd.campaigns || []); setPlacements(pd.placements || []); setTransactions(td.transactions || []); setAnalytics(nd.summary || null);
  ...
  return (
    <div dir={dir} className="space-y-7 p-4 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-xs font-black uppercase tracking-[.2em] text-emerald-600">PandaMarket Ads</p><h1 className="mt-1 text-3xl font-black text-slate-950">Advertising center</h1><p className="mt-2 text-sm text-slate-500">Sponsor products, services, content, or your entire brand.</p></div>
        <div className="flex gap-2"><button onClick={() => setRefilling(true)} className="rounded-xl border border-emerald-600 bg-white px-5 py-3 text-sm font-black text-emerald-700">Refill Ads account</button><button onClick={() => setCreating(true)} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-emerald-600/20"><Plus className="h-4 w-4" /> New campaign</button></div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[['Available balance',money(account?.balance,account?.currency),WalletCards],['Reserved',money(account?.reserved_balance,account?.currency),WalletCards],['Total spend',money(account?.total_spend,account?.currency),BarChart3],['Active campaigns',String(account?.active_campaigns || 0),Megaphone]].map(([label,value,Icon]) => <div key={String(label)} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><Icon className="h-5 w-5 text-emerald-600" /><p className="mt-4 text-xs font-bold uppercase text-slate-400">{String(label)}</p><p className="mt-1 text-2xl font-black text-slate-900">{String(value)}</p></div>)}
      </div>
