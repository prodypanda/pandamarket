...
function getFeatureValue(plan: Record<string, unknown>, key: string): string | boolean | undefined {
...
export default async function PricingPage() {
  ...
  const plans = await fetchEnabledSubscriptionPlans(backendUrl);
        ...
        {plans.length === 0 ? (
          ...
          <div
            ...
            className={`rounded-xl border-2 bg-white p-5 ${
              plan.highlight ? `${classes.primaryBorder} shadow-lg ${isAliExpress ? 'shadow-orange-900/10' : 'shadow-[#16C784]/10'}` : 'border-gray-200'
            }`}
          >
            {plan.highlight && (
            ...
            <ul className="space-y-2 mb-4">
              {features.map((f) => {
                const val = getFeatureValue(plan as unknown as Record<string, unknown>, f.key);
            ...
            <tr>
              ...
              {plans.map((plan) => (
                <th key={plan.id} className="p-4 text-center">
                  <div className={`rounded-xl p-4 ${plan.highlight ? popularPanelClass : ''}`}>
                    {plan.highlight && (
