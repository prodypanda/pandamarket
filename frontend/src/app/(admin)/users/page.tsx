...
export default function AdminVendorAccountsPage() {
  ...
  const metricCards = [
    ...
    { label: 'Active accounts', value: summary.active, icon: ShieldCheck, tone: 'from-emerald-500 to-teal-500 text-white' },
    { label: 'Inactive accounts', value: summary.inactive, icon: Ban, tone: 'from-red-500 to-rose-600 text-white' },
    { label: 'Multi-store accounts', value: summary.multi_store_accounts, icon: Store, tone: 'from-violet-500 to-indigo-600 text-white' },
    { label: 'Total stores', value: summary.total_stores, icon: Store, tone: 'from-blue-500 to-cyan-500 text-white' },
                      ...
                      <div className="flex gap-4">
                        ...
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className={`rounded-full px-3 py-1 text-xs font-black ring-1 ${account.is_active === false ? 'bg-red-50 text-red-700 ring-red-100' : 'bg-emerald-50 text-emerald-700 ring-emerald-100'}`}>
                              {account.is_active === false ? 'Inactive' : 'Active'}
                            </span>
                            <span className={`rounded-full px-3 py-1 text-xs font-black ring-1 ${account.two_factor_enabled ? 'bg-violet-50 text-violet-700 ring-violet-100' : 'bg-gray-100 text-gray-600 ring-gray-200'}`}>
