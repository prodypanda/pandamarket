...
const actorRoleOptions = ['all', 'customer'];
...
export default function BuyerAuditLogPage() {
  ...
  const fetchAuditLog = useCallback(async () => {
    ...
    setError('');
    try {
      const listParams = new URLSearchParams({ page: String(page), limit: '25', log_type: 'buyer' });
      const summaryParams = new URLSearchParams({ log_type: 'buyer' });
  ...
  const handleExport = async () => {
    ...
    setError('');
    try {
      const params = new URLSearchParams({ log_type: 'buyer' });
  ...
  const handlePurge = async () => {
    ...
    try {
      const res = await fetchWithCsrf('/api/pd/admin/audit-log/purge', {
        ...
        body: JSON.stringify({ older_than_days: purgeDays, log_type: 'buyer' }),
        credentials: 'include',
      });
      ...
      <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#3B0D0D] via-[#7F1D1D] to-[#B91C1C] p-8 text-white shadow-2xl shadow-slate-900/20">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <h1 className="text-3xl font-black tracking-tight md:text-4xl">Buyer Audit Log</h1>
            <p className="mt-3 text-sm leading-6 text-slate-300">
              Review every buyer action, order placement, and account update.
            </p>
          </div>
        ...
        <aside className="space-y-4">
          ...
          <div className="rounded-[1.5rem] border border-emerald-100 bg-emerald-50 p-5 text-sm text-emerald-900">
            ...
            <p className="text-xs leading-5">
              State-changing buyer API calls are captured with redacted request bodies, status codes, duration, actor role, IP, and user agent.
            </p>
          </div>
        </aside>
      </div>
