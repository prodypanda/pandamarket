...
- [x] Define a versioned quote contract: `quote_id`, `quote_version`, `issued_at`, `expires_at`, currency, normalized lines, per-line prices/discounts, coupon breakdown, shipping by fulfillment group, tax, and grand total. The v1 response exposes `issued_at`, and forward versions are rejected with `PD_ORDER_QUOTE_VERSION_UNSUPPORTED`.
...
- [x] Store a tamper-evident quote snapshot or digest. Do not use browser totals as an input to order persistence. Quote v1 uses a stable SHA-256 commerce-snapshot digest; privileged database mutation is explicitly outside the checkout trust boundary, so keyed HMAC is reserved for a future quote version if that threat model changes.
- [x] Persist applied coupon code and a structured discount breakdown on the order and order items.
- [x] Add quote expiry and reject expired, unknown, already-consumed, or version-mismatched quotes with a machine-readable error. Unsupported quote versions return `PD_ORDER_QUOTE_VERSION_UNSUPPORTED` with the supported version list.
...
- [-] Make quote consumption idempotent so retries cannot create a second order or consume a discount twice. Conflict-safe insertion, transaction-scoped idempotency serialization, and buyer/quote/gateway binding are covered by focused concurrent tests; disposable-PostgreSQL acceptance remains open.
...
- [-] Acceptance: focused route/service adversarial tests prove that changed client prices, shipping, coupon values, deleted products, and stale quotes cannot alter the persisted total. Run the same suite against disposable PostgreSQL/Redis under Node 20 before closing this gate.
...
**P0-01 hardening evidence (2026-08-24):** Quote v1 now exposes the database `created_at` value as `issued_at`, centralizes supported-version enforcement, and returns `PD_ORDER_QUOTE_VERSION_UNSUPPORTED` for forward versions. SHA-256 is documented as the v1 deterministic snapshot digest, with privileged database mutation outside the checkout trust boundary and HMAC reserved for a future version with key rotation. Checkout idempotency is buyer/quote/gateway-bound, length-bounded, serialized by a transaction-scoped PostgreSQL advisory lock, and protected by conflict-safe insertion; simultaneous requests return one `201` creation and one `200` replay while consuming inventory and the quote once. Focused backend coverage passes 26 tests across quote, concurrency, route, and order suites; frontend quote/accessibility coverage passes 19 tests. Backend type-check/build and changed-source lint pass; frontend type-check, changed-source lint, and the 99-route production build pass. The full backend runtime gate remains pending because this workstation has Node 24 instead of required Node 20 and no reachable disposable PostgreSQL/Redis services.
...
- [-] Acceptance: focused unsupported-gateway, changed-capability, duplicate-submit, provider-timeout, compensation, reconciliation, and callback-replay tests pass. HTTP route coverage now verifies duplicate Hub/storefront submits preserve the idempotency key and missing keys are rejected; a disposable database migration run remains before closing the item.
