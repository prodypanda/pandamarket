# 02 — Critical Bugs (P0) — Immediate Platform Blockers
...
> **Format:** What is Broken ➔ Forensic Code Evidence ➔ Root Cause ➔ Step-by-Step Fix ➔ Verification Test.
...
- **Forensic Evidence:**
  ...
  - **Database Proof:** Out of all historical rows in `pd_wallet_transaction`, only 1 sale row exists, and it was generated by the manual Mandat path (`mandat.service.ts:149`). All credit card payments result in zero wallet balance for merchants.
- **Consequences:** Merchants are never paid for credit card transactions; platform commissions are never registered; buyers of digital serial products never receive their software keys; and confirmation emails never send.
...
## [P0-4] Backend TypeScript Compile Failure: Unimported `retentionRouter` in `main.ts`

- **Severity:** `CRITICAL (P0)` — Prevents backend build and continuous deployment.
...
- **Forensic Evidence:**
  - Running `npm run type-check -w backend` fails with:
    ...
    ```
  - `retentionRouter` was mounted at `apiRouter.use('/retention', retentionRouter);` in `main.ts:345`, but the import statement was forgotten at the top of the file.
...
- **Severity:** `CRITICAL (P0)` — Transactional email worker crashes.
...
- **Forensic Evidence:**
  ...
  - In `backend/src/workers/email.worker.ts:399-404`:
    ...
    ```
  - In production, BullMQ email jobs fail hard when attempting to send password resets, OTPs, or order confirmations via SMTP.
...
- **Forensic Evidence:**
  ...
  ```
  - In non-production environments (or production if `NODE_ENV` is unset/misconfigured), unverified webhooks are accepted, capturing orders with fake payloads.
...
2. Wrap both sinks:
   ...
   ```
3. Create a standardized `<SafeAiHtml content={...} />` component to avoid future regressions.
