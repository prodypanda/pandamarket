# 13 — Master Actionable TODO Checklist
...
- [ ] **[AI-1]** ⚡ Remove error swallow on empty AI wallet; reject with `AI_INSUFFICIENT_TOKENS`.
- [ ] **[AI-2]** ⚡ Fix seller token balance display property name mismatch on products page.
- [ ] **[AI-3]** ⚡ Remove hardcoded Unsplash fallback in photo studio and only deduct tokens on success.
- [ ] **[AI-S5]** 🛠 Route photo studio image URLs through `utils/ssrf.ts` to block internal IP and cloud metadata SSRF.
- [ ] **[CMS-1]** ⚡ Add `/cms` navigation entry under "CATALOG & CONTENT" in `(admin)/layout.tsx`.
- [ ] **[CMS-2]** ⚡ Replace bare `fetch()` with `fetchWithCsrf()` in admin CMS pages.
- [ ] **[STF-1]** 🛠 Fix live draft synchronization in fullscreen theme customizer.
- [ ] **[STF-P1]** 🛠 Require purchase verification in `pd_theme_purchase` before activating premium themes.
...
- [ ] **[SET-B1]** 🛠 Add editors for the 4 missing Mandat bank keys (`mandat_bank_name`, `mandat_bank_rib`, `mandat_bank_iban`, `mandat_recipient_phone`).
- [ ] **[CMS-M1]** 🛠 Seed default Tunisian legal CMS pages (Terms, Privacy, Refund, Help Center) to eliminate `/hub/search` default links.

### Frontend Polish & Theming
...
- [ ] **[STF-M1]** 🏗 Build modular `<ThemeSections />` renderer for testimonials, newsletters, FAQs, and banners across all themes.
...
- [ ] **[ENH-2b]** 🏗 Decompose `frontend/src/app/(admin)/settings/page.tsx` (6,246 lines) into 7 isolated tab modules.
