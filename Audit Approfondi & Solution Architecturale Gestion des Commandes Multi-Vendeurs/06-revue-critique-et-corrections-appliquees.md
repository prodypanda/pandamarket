...
## 7. Complément d'implémentation — 31/08/2026 (2e passe)

### 7.1 Page storefront « Mes commandes » corrigée et enrichie

**Bug utilisateur trouvé pendant la revue** (`store/[storeHost]/account/orders/page.tsx`, ligne 78) :

```ts
const isPaid = order.payment_status === 'paid' || order.status === 'completed';
```

Les deux littéraux sont **inexistants** dans le domaine : `payment_status` vaut `pending | captured | failed | refunded` (jamais `'paid'`) et `status` n'a pas de valeur `'completed'`. `isPaid` était donc **toujours faux** : toute commande storefront — même payée et livrée — affichait indéfiniment le badge ambre « En attente ». Même famille de défaut que les littéraux SQL invalides corrigés côté backend (P2-15 de l'audit combiné), mais ici directement visible par l'acheteur.

Corrections et ajouts :

| Élément | Avant | Après |
|---|---|---|
| Badge de statut | Binaire payé/en attente, **toujours bloqué sur « En attente »** | Statut réel sur les 9 états (libellés + couleurs + icônes) **et** badge de paiement distinct (`captured` → « Payée ») |
| Colis | Aucun affichage | Carte d'expédition : statut du colis, transporteur, n° de suivi, dates d'expédition/livraison, lien **« Suivre mon colis ↗ »** (Aramex, La Poste, DHL, FedEx, UPS) |
| Articles | Liste plate tronquée à 3 | Articles regroupés dans le colis + section de repli « Articles numériques (sans expédition) » + bouton de téléchargement pour les produits numériques payés |
| Reçu Mandat | Lien affiché sur **toutes** les commandes | Affiché uniquement si `payment_gateway = manual_mandat` **et** paiement non capturé |
| Identifiant | `pd_order_...` complet | Forme courte `#DGQ97HRZ` |
| Modale de détail | Articles + total | Colis complet + sous-total / livraison / total, défilable |

Choix de conception : une commande storefront est **toujours mono-boutique** (le checkout rejette tout produit n'appartenant pas à la boutique — `order.service.ts`, garde `opts.store_id && product.store_id !== opts.store_id`), donc **un seul colis** : pas de numérotation « Colis 1/2 » comme sur le Hub. Les branches `partially_*` sont néanmoins gérées par défense.

### 7.2 Séparation stricte des canaux (confirmée et verrouillée par des tests)

Les deux canaux sont indépendants **par construction** et cette isolation est désormais couverte par des tests :

| Canal | Colonne porteuse | Endpoint | Portée |
|---|---|---|---|
| Marketplace (Hub) | `pd_order.customer_id` | `GET /api/pd/orders/me` | Toutes les boutiques de la commande (multi-colis) |
| Storefront | `pd_order.storefront_customer_id` | `GET /api/pd/orders/storefront/me` | **Portée boutique** (`AND f.store_id = $2`) |

Vérification en production (lecture seule) : **0 commande** ne porte les deux identifiants — les jeux de données sont réellement disjoints (14 marketplace, 6 sans identifiant client : commandes invité/legacy mandat & COD).

### 7.3 Bug corrigé dans `getBuyerOrderDetail` (trouvé par le nouveau test)

La portée boutique construisait le placeholder sans le préfixe `# 06 — Revue Critique & Corrections Appliquées (Zhipu GLM 3.7 / opencode, 31/08/2026)
...
---
 ...
 (`i.store_id = 2` au lieu de `i.store_id = $2`), ce qui produisait `operator does not exist: character varying = integer` — donc une **erreur 500 sur `GET /orders/storefront/:id`** et sur `GET /orders/:id` consulté par un vendeur. Corrigé et couvert par un test dédié.

### 7.4 Parcours QA-01 → QA-06 automatisé

Nouveau fichier `backend/src/__tests__/multi-vendor-qa-walkthrough.integration.test.ts` (11 tests, PostgreSQL réel) : il crée une vraie commande marketplace à deux boutiques **et** une commande storefront, puis déroule la séquence complète en vérifiant à chaque étape **l'agrégat de la commande maître ET la charge utile acheteur** (contenu de `fulfillments` : statut, transporteur, n° de suivi, dates, articles du colis).

| Test | Étape | Attendu vérifié |
|---|---|---|
| QA-01 | Commande passée | `pending`, 2 colis `pending`, chaque colis ne contient que ses propres articles |
| QA-02 | Boutique A prépare | `processing`, seul le colis A change |
| QA-03 | Boutique A expédie (Aramex) | **`partially_shipped`**, colis A porte transporteur + n° de suivi + date, colis B intact et sans fuite d'info |
| QA-04 | Boutique B expédie | `fulfilled` |
| QA-05 | Boutique A livrée | **`partially_delivered`** (et non `delivered`) |
| QA-06 | Boutique B livrée | `delivered` |
| +5 tests | Séparation des canaux | La liste marketplace ne renvoie jamais la commande storefront et réciproquement ; l'acheteur storefront ne voit **qu'un** colis (le sien) ; COD non capturé reste `payment_required` puis progresse ; `getBuyerOrderDetail` renvoie 2 colis pour le Hub et 1 seul en portée boutique |

Ce qu'il reste hors de portée d'un test : le rendu visuel réel dans le navigateur et le déclenchement depuis les boutons du tableau de bord vendeur. Le parcours cliquable (QA manuel) reste donc utile, mais toute la logique métier et la charge utile API sont désormais verrouillées.

État des suites : **backend 140 fichiers / 1618 tests verts**, **frontend 68 / 572 verts**, `tsc --noEmit` propre, ESLint 0 erreur.

---

## 8. Reste à faire (non bloquant)

- **QA manuel cliquable** : parcours navigateur à deux boutiques (nécessite une session acheteur + deux sessions vendeur). Toute la logique est déjà couverte par `multi-vendor-qa-walkthrough.integration.test.ts`.
- **i18n** des pages acheteur (Hub et storefront) : libellés en dur en français, ces pages n'utilisent pas encore `useLocale`. Chantier distinct déjà listé dans l'audit combiné.
- **Vignettes** : `<img>` brut conservé (cohérent avec le reste des pages) ; migration vers `next/image` = chantier séparé (domaines + tailles).
